...
One package I need to use frequently is `RSQLServer`. The version on CRAN depends on `dplyr 0.4.3`, version 0.5.0 has major backend updates. To install version 0.4.3 use this code. Alternatively, I could use `devtools` to install the development version of `RSQLServer`, but would rather install the stable version.
